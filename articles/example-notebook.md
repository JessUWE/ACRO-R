...
First of all, we need to call acro_init() to initialise an acro object.
This function takes the parameter suppress which can be TRUE or FALSE to
choose whether to automatically apply suppression to the results or not.
...
- The dataset used in this example notebook is the nursery dataset from
  OpenML.
- The code below reads the data from a folder called “nursery_data”
  which we assume is at the same level as the folder where you are
  working.
- The path might need to be changed if the data has been downloaded and
  stored elsewhere.
...
- Convert the children column to integers, replacing ‘more’ with random
  int from range 4-10
...
In this example a different data set will be used. The lung dataset from
the survival package is used.
...
This is an example of logit regression using ACRO We use a different
combination of variables from the original dataset.
...
This function can be used to rename the outputs to a more descriptive
name.
...
This function is used to add comments to the outputs. It can be used to
provide a description or to pass additional information to the output
checkers.
...
- The SDC analysis for each output is saved to a json file or Excel file
  (depending on the extension of the name of the file provided as an
